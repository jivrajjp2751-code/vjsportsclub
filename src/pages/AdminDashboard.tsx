                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
                            <div className="flex items-center gap-3">
                                ...
                                <h1 className="font-display text-2xl sm:text-4xl text-foreground break-words">Admin Dashboard</h1>
                            </div>
                            <Button variant="secondary" onClick={async () => { await signOut(); navigate("/auth"); }} className="flex items-center gap-2 w-full sm:w-auto">
                                            ...
                                            <div>
                                                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-1">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded shrink-0">{b.status}</span>
                                                    </div>
                                                </div>
                                    ...
                                    {filteredUsers.map((u) => (
                                        <div key={u.id} className="glass rounded-lg p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                            <div className="min-w-0">
                                                <span className="font-medium text-foreground block sm:inline">{u.display_name || "No name"}</span>
                                                <span className="text-xs text-muted-foreground sm:ml-3 block sm:inline break-all">{u.email}</span>
                                            </div>
                                            <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
                                                <span className={`text-xs px-2 py-1 rounded font-medium shrink-0 ${u.role === "admin" ? "bg-accent/20 text-accent" : "bg-secondary text-secondary-foreground"
                                                    }`}>
                                                    {u.role}
                                                </span>
                                                <Button
